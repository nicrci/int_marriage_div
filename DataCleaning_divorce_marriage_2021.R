...
mix_di_110_list <-list.files("divorce110")

mix_di_110 <-data.frame()

for (i in 1:length(mix_di_110_list)) {
  file_path <- paste0("divorce110/", mix_di_110_list[i])
  temp_df <- read.csv(file_path)
  mix_di_110 <- rbind(mix_di_110, temp_df)
}


colnames(mix_di_110) <- c("yearmonth", "district_code", "city_district", "village_li", "marriage_type", "sex", "original_nationality", "household_registration_status", "divorce_count")
...
mask <- grepl("統計年月", mix_di_110$yearmonth)

# 使用這個 mask 來過濾出不包含 '統計年月' 的行
mix_di_110_filtered <- mix_di_110[!mask, ]


mix_di_110_test2 <- mix_di_110_filtered %>%
  group_by(city_level) %>%
  summarise(
    divorce_count_sum = sum(divorce_count, na.rm = TRUE)
  )
...
mix_di_110_filtered <- mix_di_110_filtered %>%
  mutate(
...
city <- regexpr("市|縣", mix_di_110_filtered$city_district)
mix_di_110_filtered$city <- substr(mix_di_110_filtered$city_district, 1, city )
mix_di_110_filtered$district <- substr(mix_di_110_filtered$city_district, city+1, nchar(mix_di_110_filtered$city_district))


# Function for change city and district to en
mix_di_110_done <- mix_di_110_filtered %>%
  mutate(
    city = recode(city,
                  "臺北市" = "Taipei City",
                  "新北市" = "New Taipei City",
...
mix_di_110_done <- mix_di_110_done %>%
  mutate(
...
mix_di_110_sum <- mix_di_110_done %>%
  group_by(city,sex) %>%
  summarise(sum_divorce_count = sum(as.numeric(divorce_count)))

ggplot(mix_di_110_sum, aes(x = city, y = sum_divorce_count, fill = sex)) +
...
mix_di_110_na <- mix_di_110_done %>%
  group_by(original_nationality,sex) %>%
  summarise(sum_divorce_count = sum(as.numeric(divorce_count)))

ggplot(mix_di_110_na, aes(x = reorder(original_nationality, -sum_divorce_count), y = sum_divorce_count, fill = sex)) +
...
# 确保只按月份分组
unique(mix_di_110_done$month)

mix_di_110_mon <- mix_di_110_done %>%
  group_by(month) %>%
  summarise(sum_divorce_count = sum(as.numeric(divorce_count), na.rm = TRUE))


ggplot(mix_di_110_mon, aes(x = month, y = sum_divorce_count)) +
  geom_col() +
...
mix_ma_110_list <-list.files("marriage110")

mix_ma_110 <-data.frame()

for (i in 1:length(mix_ma_110_list)) {
  file_path <- paste0("marriage110/", mix_ma_110_list[i])
  temp_df <- read.csv(file_path)
  mix_ma_110 <- rbind(mix_ma_110, temp_df)
}


colnames(mix_ma_110) <- c("yearmonth", "district_code", "city_district", "village_li", "marriage_type", "sex", "original_nationality", "household_registration_status", "marry_count")


mix_ma_110 <- mix_ma_110 %>%
  mutate(city_level = case_when(
    city_district %in% level_1_districts ~ 1,
    city_district %in% level_2_districts ~ 2,
    city_district %in% level_3_districts ~ 3,
...
mask <- grepl("統計年月", mix_ma_110$yearmonth)

# 使用這個 mask 來過濾出不包含 '統計年月' 的行
mix_ma_110_filtered <- mix_ma_110[!mask, ]


mix_ma_110_test2 <- mix_ma_110_filtered %>%
  group_by(city_level) %>%
  summarise(
    marry_count_sum = sum(marry_count, na.rm = TRUE)
  )
...
mix_ma_110_filtered <- mix_ma_110_filtered %>%
  mutate(
    year = str_extract(yearmonth, "^\\d{3}"),   # 使用正則表達式提取前三位數字作為年份
    month = str_extract(yearmonth, "\\d{2}$")  # 使用正則表達式提取最後兩位數字作為月份
  )
...
# Function for divide city and district
city <- regexpr("市|縣", mix_ma_110_filtered$city_district)
mix_ma_110_filtered$city <- substr(mix_ma_110_filtered$city_district, 1, city )
mix_ma_110_filtered$district <- substr(mix_ma_110_filtered$city_district, city+1, nchar(mix_ma_110_filtered$city_district))



# Function for change city and district to en
mix_ma_110_done <- mix_ma_110_filtered %>%
  mutate(
    city = recode(city,
                  "臺北市" = "Taipei City",
                  "新北市" = "New Taipei City",
...
mix_ma_110_done <- mix_ma_110_done %>%
  mutate(
    district = recode(district,
                      "中正區"="Zhongzheng Dist.",
                      "大同區"="Datong Dist.",
...
mix_ma_110_sum <- mix_ma_110_done %>%
  group_by(city,sex) %>%
  summarise(sum_marry_count = sum(as.numeric(marry_count)))

ggplot(mix_ma_110_sum, aes(x = city, y = sum_marry_count, fill = sex)) +
  geom_col(position = position_dodge()) +  # 使用 position_dodge() 來分隔不同性別的柱子
  theme(axis.text.x = element_text(angle = 90, hjust = 1))  # 將 x 軸的文字旋轉 90 度

mix_ma_110_na <- mix_ma_110_done %>%
  group_by(original_nationality,sex) %>%
  summarise(sum_marry_count = sum(as.numeric(marry_count)))

ggplot(mix_ma_110_na, aes(x = reorder(original_nationality, -sum_marry_count), y = sum_marry_count, fill = sex)) +
  geom_col(position = position_dodge()) +
  scale_y_log10() +  # 使用對數尺度，如果仍需要的話
...
# 确保只按月份分组
unique(mix_ma_110_done$month)

mix_ma_110_mon <- mix_ma_110_done %>%
  group_by(month) %>%
  summarise(sum_marry_count = sum(as.numeric(marry_count), na.rm = TRUE))


ggplot(mix_ma_110_mon, aes(x = month, y = sum_marry_count)) +
  geom_col() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
names(merge_di_110) 
names(merge_ma_110)
class(merge_di_110$district_code)
class(merge_ma_110$district_code)

summary(merge_di_110)
summary(merge_ma_110)
...
mix_110_all <- left_join(merge_di_110, merge_ma_110, 
...
write.csv(mix_110_all, "mix_110_all.csv", row.names = FALSE)
...
mix_110_all_sum <- mix_110_all %>%
  filter(original_nationality != "Taiwan") %>%
  group_by(city, sex) %>%
  summarise(sum_divorce_count = sum(as.numeric(divorce_count), na.rm = TRUE), .groups = 'drop')

ggplot(mix_110_all_sum, aes(x = city, y = sum_divorce_count, fill = sex)) +
  geom_col(position = position_dodge()) +  # 使用 position_dodge() 來分隔不同性別的柱子
  theme(axis.text.x = element_text(angle = 90, hjust = 1))  # 將 x 軸的文字旋轉 90 度


mix_110_all_na <- mix_110_all %>%
  filter(original_nationality != "Taiwan") %>%
  group_by(original_nationality,sex) %>%
  summarise(sum_divorce_count = sum(as.numeric(divorce_count)))

ggplot(mix_110_all_na, aes(x = reorder(original_nationality, -sum_divorce_count), y = sum_divorce_count, fill = sex)) +
  geom_col(position = position_dodge()) +
  scale_y_log10() +  # 使用對數尺度，如果仍需要的話
  labs(x = "Original Nationality", y = "Sum of Divorce Count") +  # 添加軸標籤
  theme(axis.text.x = element_text(angle = 90, hjust = 1))  # 將 x 軸的文字旋轉 90 度

mix_110_all_nama <- mix_110_all %>%
  filter(original_nationality != "Taiwan") %>%
  group_by(original_nationality,sex) %>%
  summarise(sum_marry_count = sum(as.numeric(marry_count)))

sum(is.na(mix_110_all_nama))

ggplot(mix_110_all_nama, aes(x = reorder(original_nationality, -sum_marry_count), y = sum_marry_count, fill = sex)) +
  geom_col(position = position_dodge()) +
  scale_y_log10() +  # 使用對數尺度，如果仍需要的話
  labs(x = "Original Nationality", y = "Sum of marry Count") +  # 添加軸標籤
  theme(axis.text.x = element_text(angle = 90, hjust = 1))  # 將 x 軸的文字旋轉 90 度
...
mix_110_all_mon <- mix_110_all %>%
  group_by(month) %>%
  summarise(sum_divorce_count = sum(as.numeric(divorce_count), na.rm = TRUE))


ggplot(mix_110_all_mon, aes(x = month, y = sum_divorce_count)) +
  geom_col() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
